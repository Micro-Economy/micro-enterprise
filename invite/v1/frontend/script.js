...
const checkCodeUrl = "http://155.138.128.168/invite/backend/CheckCode.php";
const addMemberUrl =  "http://155.138.128.168/invite/backend/AddMember.php";
...
function isCorrectInput(code){
    ...
    $.ajax(settings).done(function (response) {
      ...
      if(response){
        returnStatus=(response.status==="success" && response.result!="Invalid Code")}
      else{
        returnStatus = false;
      }
    })
